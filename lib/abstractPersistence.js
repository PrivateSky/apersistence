...
var security = require('./cryptographic.js')
...
function AbstractPersistence(persistenceStrategy){
    ...
    this.saveFields = function(obj, fields, callback){
        ...
        fields.forEach((field) => {
            ...
            if(objModel.isTransient(field)){
                ...
                securityModel = objModel.getSecurityModel(field);
                fieldType = objModel.getFieldType(field);
                var password = undefined
                if(passwords !== undefined && passwords[field] !== undefined) {
                    password = passwords[field]
                }
                security.encrypt(obj[field], fieldType, securityModel, password, function(err, res) {
                    obj[field] = res;    
    ...
    this.filter = function(typeName, filter, callback){
        ...
        var passwords = undefined;
        if(filter.__passwords !== undefined) {
            passwords = filter.__passwords;
            delete filter.__passwords
        } 
        var serializedFilter = {}
        var objModel = modelUtil.getModel(typeName);
        var processed = 0;
        var sign = '';
        for(var field in filter){
            ...
            if(isComparison(filter[field])) {
                sign = filter[field].split(/[^<>=!]/)[0].replace(' ', '');
                if(['int', 'float', 'number'].indexOf(fieldType) != -1)
                    value = Number(filter[field].replace(sign, '').replace(' ', ''));
                else
                    value = filter[field].replace(sign, '');
            } else {
                value = filter[field]
            }
            ...
            if(passwords !== undefined) {
                if(passwords[field] !== undefined) {
                    password = passwords[field]
                }
            }
            security.encrypt(value, fieldType, securityModel, password, function(err, res) {
                f = sign + res
                serializedFilter[field] = modelUtil.serialiseField(typeName,field,f,persistenceStrategy);
            });
            if(processed == Object.keys(filter).length) {
                var processed_res = 0;
                persistenceStrategy.filter(typeName, serializedFilter, function(err, res) {
                    if(res.length == 0) {
                        return callback(null, res)
                    }
                    res.forEach((result)=> {
                        processed_res++;
                        for(var field in result.__meta.savedValues) {
                            securityModel = objModel.getSecurityModel(field);
                            fieldType = objModel.getFieldType(field);
                            var password = undefined;
                            if(passwords !== undefined && passwords[field] !== undefined) {
                            ...
                            security.decrypt(result.__meta.savedValues[field], fieldType, securityModel, password, function(err,res) {
                                result.__meta.savedValues[field] = res
                                result.__meta[field] = res
                            })
                        }
                        if(processed_res == res.length) {
                            callback(null, res)
                        }
