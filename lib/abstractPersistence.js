...
function AbstractPersistence(persistenceStrategy){
    ...
    this.saveFields = function(obj, fields, callback){
        ...
        fields.forEach(function(field){
            if(objModel.isLazy(field)){
                var lazyType = objModel.getFieldDescription(field).type;
                var isArray = false;
                if(lazyType.match("array")){
                    lazyType = lazyType.split(":")[1];
                    isArray = true;
                }

                if(isArray && obj[field].length===0){
                    valueDiff.push(modelUtil.serialiseField(obj.__meta.typeName, field, obj[field], persistenceStrategy))
                    return;
                }

                var isLoaded = isArray?(obj[field][0].__meta!==undefined):(obj[field].__meta!==undefined)
                var lazyModel= modelUtil.getModel(lazyType);


                if(!isArray&&!isLoaded){
                    valueDiff.push( modelUtil.serialiseField(lazyType, lazyModel.getPKField(), obj[field], persistenceStrategy));
                
                }else if(!isArray &&isLoaded){
                    valueDiff.push( modelUtil.serialiseField(lazyType,lazyModel.getPKField(),obj[field].__meta.getPK(), persistenceStrategy));
                
                }else if(isArray && !isLoaded){
                    var valArr = modelUtil.serialiseField(obj.__meta.typeName, field, obj[field], persistenceStrategy);
                    valueDiff.push(valArr);
                
                }else if(isArray && isLoaded){
                    var valArr = obj[field].map(function(arrObj){return arrObj.__meta.getPK();});
                    valArr = modelUtil.serialiseField(obj.__meta.typeName, field, valArr, persistenceStrategy)
                    valueDiff.push(valArr);
                }
            }else{
                valueDiff.push( modelUtil.serialiseField(obj.__meta.typeName,field,obj[field], persistenceStrategy));
    ...
    this.registerModel = function(typeName,model,callback){
        ...
        var dependencies = dependentFields.map(function(field){
            if(field.type.match("array")){
                return field.type.split(":")[1]+"PersistenceModel";
            }else {
                return field.type + "PersistenceModel";
            }
        });

        container.declareDependency(typeName+"PersistenceModel",dependencies,function(outOfService){
            if(!outOfService){
                ...
                validateModel(databaseModel,typeName,function(err,result){
                    if(err){
                        ...
                        finalizeRegistration(model,typeName,callback);
                    }
                });
        ...
        function finalizeRegistration(model,typeName,callback){
            if (persistenceRegistry[typeName] === undefined)
                persistenceRegistry[typeName] = [];
            persistenceRegistry[typeName].push(self);
            var model = modelUtil.registerModel(typeName, model, self.persistenceStrategy);
            callback(null, model);
            container.resolve(typeName+"PersistenceModel",model);
        }
    };
